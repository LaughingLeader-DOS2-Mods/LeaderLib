package
{
	...
	{		
		...
		public function launchTimer(delay:Number, callback:String, repeat:int=1) : *
		{
			var timer:Timer = new Timer(delay, repeat);
			timer.addEventListener(TimerEvent.TIMER_COMPLETE, function(event:TimerEvent):void {
				ExternalInterface.call("LeaderLib_TimerComplete", callback);
			});
			if (repeat > 1)
			{
				timer.addEventListener(TimerEvent.TIMER, function(event:TimerEvent):void {
					ExternalInterface.call("LeaderLib_TimerTick", callback);
				});
			}
			timer.start();
		}
