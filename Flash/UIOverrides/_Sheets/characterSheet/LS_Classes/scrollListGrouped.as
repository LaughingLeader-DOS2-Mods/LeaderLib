package LS_Classes
{
	...
	public class scrollListGrouped extends listDisplay
	{
		...
		public function scrollListGrouped(param1:String = "down_id", param2:String = "up_id", param3:String = "handle_id", param4:String = "scrollBg_id", param5:String = "", param6:String = "")
		{
			...
			var val7:Sprite = new Sprite();
			val7.graphics.lineStyle(0,16777215);
			val7.graphics.beginFill(16777215);
			val7.graphics.drawRect(0,0,1,1);
			val7.graphics.endFill();
			this.m_frameHit_mc.addChild(val7);
			val7.alpha = 0;
		...
		public function addGroup(groupId:Number, labelText:String, reposition:Boolean = true) : MovieClip
		{
			var group_mc:MovieClip = getElementByNumber("groupId",groupId);
			if(group_mc == null)
			{
				group_mc = new this.groupClass();
				group_mc.list = new listDisplay();
				group_mc.list.m_cyclic = false;
				group_mc.list.EL_SPACING = this.m_SubElementSpacing == -1?EL_SPACING:this.m_SubElementSpacing;
				if(group_mc.listContainer_mc == null)
				{
					group_mc.listContainer_mc = new MovieClip();
					group_mc.addChild(group_mc.listContainer_mc);
				}
				group_mc.listContainer_mc.addChild(group_mc.list);
				addElement(group_mc,reposition);
				group_mc._isOpen = false;
				group_mc.groupId = groupId;
				group_mc.mainList = this;
				group_mc.isNew = true;
				if(this.m_ElSortOnFieldName)
				{
					group_mc.list.sortOn(this.m_ElSortOnFieldName,this.m_ElSortOnOptions);
				}
			}
			else
			{
				group_mc.isNew = false;
			}
			if(group_mc && group_mc.title_txt != null)
			{
				group_mc.title_txt.htmlText = labelText;
				group_mc.groupName = labelText;
			}
			return group_mc;
		}
		
		public function addGroupElement(param1:Number, param2:MovieClip, reposition:Boolean = true) : MovieClip
		{
			...
			if(val4)
			{
				...
				param2.grpMc = val4;
				val4.list.addElement(param2,reposition);
				if(reposition)
				{
					this.positionElements();
				}
		...
		override public function cleanUpElements() : *
		{
			...
			while(val1 < content_array.length)
			{
				content_array[val1].list.cleanUpElements();
				if(this.m_HideEmptyGroups)
				{
					content_array[val1].visible = content_array[val1].list.visibleLength > 0;
				}
				val1++;
			}
		...
		override public function positionElements() : *
		{
			...
			while(val1 < content_array.length)
			{
				content_array[val1].list.positionElements();
				if(content_array[val1].setGroupAmount)
				{
					content_array[val1].setGroupAmount(content_array[val1].list.visibleLength);
				}
				val1++;
			}
			this.positionElements2();
		...
		public function positionElements2() : *
		{
			...
			while(val2 < content_array.length)
			{
				...
				if(canPositionInvisibleElements || content_array[val2].visible)
				{
					val1 = val1 + (this.getElementHeight(content_array[val2]) + EL_SPACING);
				}
				val2++;
			}
			this.checkScrollBar();
		...
		override public function next() : *
		{
							...
							if(val2.INTSelectNext == null || !val2.INTSelectNext())
							{
								...
								else
								{
									...
									while(val3 < content_array.length)
									{
										...
										if(val4 && val4.visible && val4._isOpen && val4.list.visibleLength > 0)
										{
			...
			else if(!this.m_SelectGroups)
			{
				...
				while(val7 < content_array.length)
				{
					...
					if(val8 && val8.visible && val8._isOpen && val8.list.length > 0)
					{
						select(val7);
			...
			if(val1 && val2)
			{
				...
				else if(val2.list_pos > 0)
				{
					...
					while(val6 >= 0)
					{
						...
						if(val7 && val7.visible && val7._isOpen && val7.list.length > 0)
						{
							val2.list.selectMC(null);
							select(val6);
					...
					if(val1 && val1 != val2)
					{
						...
						else
						{
							...
							if(this.m_CustomGroupElementHeight)
							{
								...
								val4 = val4 + val2.m_GroupElementHeight;
							}
							else
							{
								val4 = val4 + val2.listContainer_mc.y;
							}
		...
		override public function getElementHeight(param1:MovieClip) : Number
		{
			...
			else
			{
				...
				{
					val4 = getElementByNumber("groupId",param1.groupId);
					if(val4)
					{
