package LS_Classes
{
	...
	public class LSSlider extends MovieClip
	{
		...
		public function LSSlider(param1:String = "notch_id")
		{
			...
			var val2:Number = this.m_bg_mc.height;
			if(this.m_left_mc)
			{
				if(val2 < this.m_left_mc.height)
				{
					val2 = this.m_left_mc.height;
				}
				this.m_left_mc.y = Math.round((val2 - this.m_left_mc.height) * 0.5);
			...
			if(this.m_right_mc)
			{
				if(val2 < this.m_right_mc.height)
				{
					val2 = this.m_right_mc.height;
				}
				this.m_right_mc.y = Math.round((val2 - this.m_right_mc.height) * 0.5);
			...
			if(this.m_handle_mc)
			{
				if(val2 < this.m_handle_mc.height)
				{
					val2 = this.m_handle_mc.height;
				}
		...
		private function setNotches(param1:Number) : *
		{
			...
			{
				val2 = getDefinitionByName(this.m_notchStr) as Class;
				this.removeChildrenOf(this.m_notches_mc);
				val3 = int((this.m_max - this.m_min) / param1) + 1;
				if(val3 > 1 && val2)
				{
					val4 = this.m_right_mc.x - this.m_left_mc.x - this.m_NotchLeftOffset - this.m_NotchRightOffset + this.m_bgToWidthDiff;
					val5 = val4 / (val3 - 1);
					val6 = 0;
					while(val6 < val3)
					{
						val7 = new val2();
						val7.x = this.m_NotchLeftOffset + val5 * val6;
						this.m_notches_mc.addChild(val7);
						val6++;
					}
				}
			}
		...
		{
			var val2:Number = param1;
			if(val2 <= 0)
			{
				val2 = 0;
			}
			else if(val2 > this.m_scrollerDiff)
			{
				val2 = this.m_scrollerDiff;
			}
			this.ScrolledX = val2;
		}
		...
		private function handlePressed(param1:Event) : *
		{
			...
			if(!this.m_disabled)
			{
				...
				dispatchEvent(new Event("handlePressed"));
			}
		}
		...
		private function handleMove(param1:Event) : *
		{
			...
			if(!this.m_disabled)
			{
				...
				else
				{
					...
					val2 = this.m_handle_mc.x - this.m_left_mc.width;
				}
