...
local function LeaderLib_Debug_OnDebugUIEvent(ui, event, ...)
	local params = {...}
	Ext.Print("[LeaderLib:BootstrapClient.lua:OnDebugUIEvent] Event (" .. tostring(event) .. ") Params(".. LeaderLib.Common.Dump(params) .. ")")
	...
	if cc ~= nil then
		...
		Ext.Print("[LeaderLib:BootstrapClient.lua:OnDebugUIEvent] Called function on characterCreation.swf.")
	else
		Ext.Print("[LeaderLib:BootstrapClient.lua:OnDebugUIEvent] Failed to get Public/Game/GUI/characterCreation.swf")
	end
end
...
local function LeaderLib_Client_OpenDebugWindow()
	...
	if ui ~= nil then
		...
		Ext.Print("[LeaderLib:BootstrapClient.lua] Showing debug window.")
		ui:Show()
