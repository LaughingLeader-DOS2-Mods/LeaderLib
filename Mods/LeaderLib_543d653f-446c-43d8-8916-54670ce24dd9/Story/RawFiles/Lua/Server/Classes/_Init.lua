Ext.Require("Server/Classes/Hit/HitData.lua")
Ext.Require("Server/Classes/Hit/HitPrepareData.lua")
Ext.Require("Server/Classes/Hit/ProjectileHitData.lua")
Ext.Require("Server/Classes/SkillEventData.lua")
Ext.Require("Server/Classes/CharacterData.lua")
Ext.Require("Server/Classes/ItemData.lua")
Ext.Require("Server/Classes/PresetData.lua")
Ext.Require("Server/Classes/QuestData.lua")
Ext.Require("Server/Classes/CallbackHandler.lua")
if coroutine then
Ext.Require("Server/Classes/Scenes/SceneStateData.lua")
Ext.Require("Server/Classes/Scenes/SceneData.lua")
end