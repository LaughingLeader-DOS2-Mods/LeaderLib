...
---@alias GetDamageFunctionCallback fun(skillData:StatEntrySkillData, attacker:StatCharacter, isFromItem:boolean, stealthed:boolean, attackerPos:number[], targetPos:number[], level:integer, noRandomization:boolean, mainWeapon:StatEntryWeapon|nil, offhandWeapon:StatEntryWeapon|nil):DamageList,DeathType|nil
...
function GameHelpers.Damage.ApplyDamage(source, target, params)
	...
	local damageType = params.DamageType or _defaultParams.DamageType --[[@as string]]
	...
	status.HitReason = Data.HitReason.OsiApplyDamage
	status.DamageSourceType = "Attack"
	status.Hit.Hit = true
	status.Hit.DamageType = damageType
	...
	if params.DamageList then
		...
		if params.FixedAmount then
			damageList = Ext.Stats.NewDamageList()
			damageList:Add(damageType, params.FixedAmount)
		else
			if params.GetDamageFunction ~= nil then
	...
	if damageList then
		...
		if GameHelpers.Ext.ObjectIsCharacter(target) and GameHelpers.Ext.ObjectIsCharacter(source) then
			if HitOverrides._ComputeCharacterHitFunction(target.Stats, source.Stats, source.Stats.MainWeapon, damageList, hitType, false, false, status.Hit, alwaysBackstab, highGroundFlag, criticalRoll) then
				if params.FixedAmount then
					damageList:Clear(damageType)
					damageList:Add(damageType, params.FixedAmount)
				end
			end
		end
		status.Hit.TotalDamageDone = 0
		status.Hit.DamageDealt = 0
		for _,damage in pairs(damageList:AsArray()) do
		...
		status.BringIntoCombat = status.Hit.TotalDamageDone > 0
		if not StringHelpers.IsNullOrEmpty(deathType) then
