...
local DamageSourceCalcTable = {
    BaseLevelDamage = function (attacker, target, level)
        return math.max(0, Ext.Round(Game.Math.GetLevelScaledDamage(level)))
    end,
    AverageLevelDamge = function (attacker, target, level)
        return math.max(0, Ext.Round(Game.Math.GetAverageLevelDamage(level)))
    end,
    MonsterWeaponDamage = function (attacker, target, level)
        return math.max(0, Ext.Round(Game.Math.GetLevelScaledMonsterWeaponDamage(level)))
    end,
...
function Game.Math.CalculateWeaponDamage(attacker, weapon, noRandomization)
    ...
    if offHand ~= nil and weapon.InstanceId ~= offHand.InstanceId and false then -- Temporarily off
        ...
        for i, bonusWeapon in pairs(bonusWeapons) do
            ...
            Game.Math.CalculateWeaponScaledDamage(attacker, bonusWeaponStats, damageList, noRandomization)
        end
    end
...
function Game.Math.GetResistance(character, type)
...
--- @param weapon StatItem
--- @param character StatCharacter
function Game.Math.GetCriticalHitMultiplier(weapon, character)
...
--- @param attacker StatCharacter
function Game.Math.ApplyCriticalHit(hit, attacker)
    local mainWeapon = attacker.MainWeapon
    if mainWeapon ~= nil then
        hit.EffectFlags = hit.EffectFlags | Game.Math.HitFlag.CriticalHit;
        hit.DamageMultiplier = hit.DamageMultiplier + (Game.Math.GetCriticalHitMultiplier(mainWeapon, attacker, 0, 0) - 1.0)
    end
end
...
function Game.Math.ShouldApplyCriticalHit(hit, attacker, hitType, criticalRoll)
    ...
    if attacker.TALENT_ViolentMagic and hitType == "Magic" then
        ...
        if (hit.EffectFlags & Game.Math.HitFlag.Backstab) ~= 0 then
            return true
        end
...
--- @param hitType string HitType enumeration
--- @param criticalRoll string CriticalRoll enumeration
function Game.Math.ConditionalApplyCriticalHitMultiplier(hit, target, attacker, hitType, criticalRoll)
...
--- @param criticalRoll string CriticalRoll enumeration
function Game.Math.ComputeCharacterHit(target, attacker, weapon, damageList, hitType, noHitRoll, forceReduceDurability, hit, alwaysBackstab, highGroundFlag, criticalRoll)
