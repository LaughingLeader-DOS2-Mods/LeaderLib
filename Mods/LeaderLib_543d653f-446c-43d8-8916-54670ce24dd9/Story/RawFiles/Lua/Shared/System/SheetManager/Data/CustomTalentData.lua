...
---@class CustomTalentData:CustomTalentDataBase
local CustomTalentData = {
...
local defaults = {
	...
	Description = "",
	Icon = "",
	IconWidth = CustomTalentData.IconWidth,
	IconHeight = CustomTalentData.IconHeight,
	Visible = true,
	TooltipType = "Talent",
	Enabled = {},
...
---@protected
function CustomTalentData.SetDefaults(data)
...
---@return integer
function CustomTalentData:GetValue(character)
...
function CustomTalentData:GetLastValue(character)
...
---@param value integer
function CustomTalentData:SetValue(character, value)
	if not isClient then
	...
	fprint(LOGLEVEL.WARNING, "[CustomTalentData:SetValue(%s, %s)] This function only works on the server-side.", self.ID, value)
	return false
end
...
function CustomTalentData:ModifyValue(character, amount)
	if not isClient then
		return CustomStatSystem:ModifyStat(character, self.ID, amount, self.Mod)
	end
	fprint(LOGLEVEL.WARNING, "[CustomTalentData:ModifyValue(%s, %s)] This function only works on the server-side.", self.ID, amount)
	return false
end
...
---@param character EsvCharacter|string|number
---@param amount integer
function CustomTalentData:AddAvailablePoints(character, amount)
	if not isClient then
		return CustomStatSystem:AddAvailablePoints(character, self, amount)
	else
		fprint(LOGLEVEL.WARNING, "[CustomTalentData:AddAvailablePoints(%s, %s, %s)] [WARNING] - This function is server-side only!", self.ID, character, amount)
	end
end
...
---@return integer
function CustomTalentData:GetAvailablePoints(character)
	if isClient then
...
---@param character EsvCharacter|EclCharacter|UUID|NETID
function CustomTalentData:UpdateLastValue(character)
	if not StringHelpers.IsNullOrWhitespace(self.UUID) then
		...
		if value then
			if Vars.DebugMode and Vars.Print.CustomStats then
				fprint(LOGLEVEL.WARNING, "[CustomTalentData:UpdateLastValue:%s] Set LastValue for (%s) to (%s) [%s]", self.Type, characterId, value, Ext.IsServer() and "SERVER" or "CLIENT")
			end
...
--setmetatable(CustomTalentData, CustomTalentData)
Classes.CustomTalentData = CustomTalentData